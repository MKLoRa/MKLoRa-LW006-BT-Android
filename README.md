# MKLoRa LW006-BT Android SDK

Native Android SDK and demo app for LW006-BT (Smart Badge) devices. Supports BLE scanning, connection, protocol parameter read/write, device-initiated disconnect notifications, BLE/WiFi/GPS positioning and motion configuration, alarm settings, beacon filter rules, storage data and log export, and Nordic DFU firmware updates.

Cross-platform reference (same protocol): [MKLoRa-LW006-BT-Flutter](https://github.com/MKLoRa/MKLoRa-LW006-BT-Flutter).
...
│   ├── LoRaLW006MokoSupport.java   # Connect, send commands, event callbacks
...
Communication has three stages: **scan → connect → command exchange**. The SDK reports connection status and command results via **EventBus** (you can switch to another bus in `LoRaLW006MokoSupport`).
...
Scan filters by Service Data UUID: `0000aa0d-...` (`OrderServices.SERVICE_ADV`).
...
scanner.startScanDevice(new MokoScanDeviceCallback() {
    ...
    public void onScanDevice(DeviceInfo deviceInfo) {
        ...
        // adv.deviceType (0=LR1110, 1=L76)
        // adv.batteryVoltage / adv.powerPercent
        // adv.verifyEnable (password required?)
        // adv.connectable / adv.txPower
    }
...
Parsed from Service Data UUID `0000AA0D` (14 bytes):

- `deviceType` — GPS module type (`0` = LR1110, `1` = L76)
- `batteryVoltage` — battery voltage (mV)
- `powerPercent` — battery percentage
...
scanner.stopScanDevice();
LoRaLW006MokoSupport.getInstance().connDevice(mac);
```
...
if (MokoConstants.ACTION_DISCOVER_SUCCESS.equals(action)) {
    ...
    LoRaLW006MokoSupport.getInstance().sendOrder(tasks.toArray(new OrderTask[]{}));
}
```
...
```java
// Single task
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.getLoraRegion());
...
```java
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setTimeZone(8));
```
...
```java
// Read
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.getLoraUploadMode());

// Write: 1=ABP, 2=OTAA
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setLoraUploadMode(2));
```

### Example 4: Change advertisement name

```java
LoRaLW006MokoSupport.getInstance().sendOrder(
        OrderTaskAssembler.setAdvName("LW006-001"));
```

### Example 5: Sync UTC time

```java
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setTime());
```
...
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.readStorageData(timestamp));

// Stop sync when done
LoRaLW006MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setSyncEnable(0));
```

Parsed records are accumulated in `LoRaLW006MokoSupport.getInstance().exportDatas` (`ExportData`). See `ExportDataActivity` for the full export flow.
...
```xml
<service android:name="com.moko.lw006.service.DfuService" />
```
...
```
Scan page (LoRaLW006MainActivity)
  ...
  └─ DeviceInfoActivity
       ...
       ├─ Position: PosBleFixActivity, PosGpsL76CFixActivity, PosGpsLR1110FixActivity, PosWifiFixActivity, PosBleAndGpsActivity
       ├─ Motion: MotionModeActivity, PeriodicModeActivity, TimingModeActivity, StandbyModeActivity
       ├─ Alarm: AlarmFunctionActivity, AlarmSosSettingActivity, AlertAlarmSettingActivity, ManDownDetectionActivity
       ├─ Filter: FilterIBeaconActivity, FilterAdvNameActivity, FilterMacAddressActivity, FilterUIDActivity, FilterUrlActivity, FilterTLMActivity, FilterBXPIBeaconActivity, FilterBXPTagIdActivity, FilterBXPButtonActivity, FilterMkPirActivity, FilterOtherActivity, FilterRawDataSwitchActivity
       ├─ ExportDataActivity → storage data sync and export
       ├─ LogDataActivity → device log sync
...
| Connect | `LoRaLW006MokoSupport` | Connect, send commands, Bluetooth on/off |
...
2. **EventBus**: The SDK posts events internally. To use LiveData/RxJava instead, change `orderFinish` / `orderTimeout` / `orderResult` / `orderNotify` in `LoRaLW006MokoSupport`.
...
5. **Demo references**: Scan/connect — `LoRaLW006MainActivity`; parameters — `LoRaConnSettingActivity`, `DeviceInfoActivity`, `DeviceFragment`; positioning — `PositionFragment`, `PosGpsL76CFixActivity`, `PosGpsLR1110FixActivity`, `PosWifiFixActivity`; storage export — `ExportDataActivity`; log data — `LogDataActivity`; DFU — `SystemInfoActivity`.
